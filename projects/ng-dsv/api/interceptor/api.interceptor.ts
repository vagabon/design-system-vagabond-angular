...
const getToken = <T>(req: HttpRequest<T>, apiService: ApiService, storageService: StorageService) => {
...
const handle401Error = <T>(
    ...
    const jwtRefresh = JSON.parse(storageService.getItem('user-connected') as string)?.[
        'jwtRefresh' as keyof {}
    ];
    return httpClient
